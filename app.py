"""
Controller Terminal v5.1 — Pure Native Streamlit
Zero custom HTML in panels — all native widgets
"""
...
STRENGTH_PAIRS = ["EUR/USD","GBP/USD","USD/JPY","AUD/USD","USD/CAD","USD/CHF","NZD/USD","GBP/JPY"]
...
def fetch_cot(asset):
    ...
    url = (f"https://publicreporting.cftc.gov/resource/6dca-aqww.json"
           f"?cftc_contract_market_code={code}&$order=report_date_as_yyyy_mm_dd DESC&$limit=2")
    try:
        r = requests.get(url, timeout=10); r.raise_for_status(); d = r.json()
        ...
        import random; random.seed(hash(asset)+int(time.time()/3600))
        cl=40000+random.randint(-10000,15000); cs=35000+random.randint(-8000,12000); net=cl-cs
...
def fetch_calendar(asset):
    ...
    url=f"https://finnhub.io/api/v1/calendar/economic?from={today}&to={ahead}&token={FINNHUB_KEY}"
    try:
